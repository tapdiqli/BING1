...
import { getStoredGclid, subscribeGclid } from "@/lib/gclid";
      ...
      <div className="flex flex-col gap-4 p-4 pt-5 sm:flex-row sm:items-center sm:gap-5 sm:p-5 sm:pt-6">
        <div className="flex items-center gap-4 sm:contents">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p
              className={`mt-1 font-bonus text-xl font-normal uppercase leading-none tracking-wide sm:text-2xl ${styles.offer}`}
            >
              {offer.offer}
            </p>
            <p className="mt-1 font-bonus text-sm font-normal uppercase tracking-wide text-white sm:text-base">
              {offer.spins}
            </p>
          </div>
        </div>
...
export function CloakingModal({ open, visitId = "" }: CloakingModalProps) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[110] flex h-dvh flex-col overflow-hidden overscroll-none bg-[#07101f]"
    >
      <div className="mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col overflow-y-auto overscroll-contain px-4 py-6 sm:px-6 sm:py-8">
